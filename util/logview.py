...
def format_experiment_logview(logfile, data_sha):
    ...
    with open(logfile, 'r') as fp:
        while True:
            ...
            if times > 600:
                ...
                exit()
    # parse
    command, health_score, work_id, instance_id, logview = lines[:5]
    first_log = lines[i]
    ...
    mode, model, conf, experiment, version, nation = command.split('--env_str=')[-1].split(',')[:6]
    oss_path = 'oss://deeprank/lzd_rerank/{}/{}/checkpoints/{}/{}'.format(nation, model, experiment, version)
    ...
    format += "**logfile** : ` logs%s ` \n" % logfile.split('logs')[1].strip()
